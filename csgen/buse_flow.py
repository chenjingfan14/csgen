...
from csgen.compress_flow import taylor_maccoll_stream_mach, theta_oblique, \
...
def busemann_M2_beta2(config):
    ...
    dtheta = config.get('dtheta', 0.05*pi/180)
    ...
    if verbosity == 1:
        print('Solving Busemann flow field...')
        width = 12
        print('-'*3*width)
        print(f"{'Step':^{width}}{'Theta':^{width}}{'Singularity':^{width}}")
        print(f"{'Number':^{width}}{'(deg)':^{width}}{'Metric':^{width}}")
        print('-'*3*width)
    i = 0
    while r.successful() and sing_metric(r.y[0], r.y[1], r.t) < 0:
        r.integrate(r.t + dt)
        if verbosity == 1 and i % print_freq == 0:
            print(f'{i:^{width}}{r.t*180/pi:^{width}.4}{sing_metric(r.y[0], r.y[1], r.t):^{width}.4}')
        thetas.append(r.t)
        us.append(r.y[0])
        vs.append(r.y[1])
        rs.append(r.y[2])
...
def busemann_M1_p3p1(config):
    ...
    dtheta = config.get('dtheta', 0.05*pi/180)
    M2_guess = config.get('M2_guess', 5.768)
    beta2_guess = config.get('beta2_guess', 0.2410)
    max_steps = config.get('max_steps', 10000)
    print_freq = config.get('print_freq', 10)
    interp_sing = config.get('interp_sing', True)
    verbosity = config.get('verbosity', 1)
    ...
    if verbosity == 1:
        ...
        width = 12
        print('-'*3*width)
        print(f"{'Iteration':^{width}}{'M1':^{width}}{'p3/p1':^{width}}")
        print(f"{'Number':^{width}}{'Residual':^{width}}{'Residual':^{width}}")
        print('-'*3*width)
    ...
    if not sol.success:
        ...
        if verbosity == 1:
            print('-'*3*width)
            print('Root finder has converged. \n')

    config['M2'] = sol.x[0]
    config['beta2'] = sol.x[1]
    config['verbosity'] = 1
    buse = busemann_M2_beta2(config)
    return buse
