...
Interpolate US Standard Atmosphere 1976 data up to 86km altitude.
...
Columns:
    ...
    6 - Density (kg/m^3)
    7 - Sonic Speed (m/s)
    8 - Dynamic Viscosity (kg/(m*s))
    9 - Kinematic Viscosity (m^2/s) 

Notes:
- Some quantities are non-increasing & non-decreasing, they do not behave well 
  when used as 'known_property':
  -> Temperature, Sonic Speed, Dynamic Viscosity and Kinematic Viscosity
"""

import numpy as np

data = [[-2, 1.2067E+0, 1.2611E+0, 1.0451, 301.2, 1.278E+5, 1.478E+0, 347.9, 18.51, 1.25E-5],
...
datadict = {'Altitude': [x[0] for x in data],
            'Temperature': [x[4] for x in data],
            'Pressure': [x[5] for x in data],
            'Density': [x[6] for x in data],
            'Sonic Speed': [x[7] for x in data],
            'Dynamic Viscosity': [x[8] for x in data],
            'Kinematic Viscosity': [x[9] for x in data]}
  
def atmos_interp(known_property_value, known_property, unknown_property):
    """
    Returns a quantity that is linearly interpolated from the US Atmosphere 1976.
    
    Arguments:
    - known_property = known property (string)
    - unknown_property = property being interpolated (string)
    - known_property_value = value of known property (float)
    
    Constraints:
    - known_property and unknown_property must be one of the strings from datadict
    """
    # extract list of values for desired known and unknown properties
    known_properties = datadict[known_property]
    unknown_properties = datadict[unknown_property]
    
    # np.interp assumes known_properties are non-decreasing
    # reverse order of lists if known_property is not non-decreasing
    if known_property in ['Pressure', 'Density']:
        known_properties = known_properties[::-1]
        unknown_properties = unknown_properties[::-1]
    
    return np.interp(known_property_value, known_properties, unknown_properties) 
